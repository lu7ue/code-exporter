...
def get_history_file(app_name="Otter"):
    import sys, os
    from pathlib import Path
    ...
    return base_dir / f"{app_name}_history.json"
...
class UI:
    ...
    def build_path_input(self, parent):
        ...
        row_frame.grid_columnconfigure(3, weight=0)  # Down Arrow Button
        row_frame.grid_columnconfigure(2, weight=0)  # OK Button
        ...
        # Down Arrow Button
        arrow_btn = tk.Button(row_frame, text="▼", width=2, command=self.show_history)
        arrow_btn.grid(row=0, column=2, padx=2)

        # OK Button
        tk.Button(row_frame, text="OK", command=self.scan).grid(row=0, column=3, padx=5)
    ...
    # -------------------------------------------------
    # Build bottom bar with export buttons
    # -------------------------------------------------
    ...
    def scan(self):
        ...
        if folder_str in self.history_paths:
            self.history_paths.remove(folder_str)
        self.history_paths.insert(0, folder_str)
        self.history_paths = self.history_paths[:5]
        try:
            self.history_file.write_text(
                json.dumps(self.history_paths), encoding="utf-8"
            )
        except:
            pass
    ...
    def show_history(self):
        ...
        if getattr(self, "history_visible", False):
            self.history_listbox.place_forget()
            self.history_visible = False
            return

        x = self.folder_entry.winfo_rootx() - self.root.winfo_rootx()
        y = (
            self.folder_entry.winfo_rooty()
            - self.root.winfo_rooty()
            + self.folder_entry.winfo_height()
        )
        self.history_listbox.place(x=x, y=y, width=self.folder_entry.winfo_width())
    ...
    # -------------------------------------------------
    # Select a history path
    # -------------------------------------------------
    def select_history(self, event):
        selection = self.history_listbox.curselection()
        if selection:
            value = self.history_listbox.get(selection[0])
            self.folder_entry.delete(0, tk.END)
